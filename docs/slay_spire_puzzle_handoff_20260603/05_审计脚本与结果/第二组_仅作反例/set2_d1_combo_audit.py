...
def apply_card(card: str, enemy: int, block: int, combo: int) -> tuple[int, int, int]:
    if card == "Strike":
        enemy -= 6
    elif card == "Guard":
        block += 5
    elif card == "Quick Stab":
        enemy -= 4
        combo += 1
    elif card == "Twin Cut":
        enemy -= 6
    ...
    return enemy, block, min(combo, 8)


@lru_cache(maxsize=None)
def play_options(hand: tuple[str, ...], discard: tuple[str, ...], enemy: int, combo: int, energy: int, block: int):
    states = {(hand, discard, enemy, combo, energy, block)}
    changed = True
    while changed:
        changed = False
        for h, disc, e, c, en, b in list(states):
            if e <= 0:
                continue
            for card in sorted(set(h), key=ORDER.index):
                ...
                ne, nb, nc = apply_card(card, e, b, c)
                ns = (nh, nd, ne, nc, en - COST[card], nb)
                if ns not in states:
                    states.add(ns)
                    changed = True
    return tuple(states)
...
def result_for(deck: tuple[str, ...], enemy_hp: int, player_hp: int, incoming: tuple[int, ...]) -> tuple[float, ...]:
    ...
    @lru_cache(maxsize=None)
    def solve(turn: int, hand: tuple[str, ...], draw: tuple[str, ...], discard: tuple[str, ...], hp: int, enemy: int, combo: int):
        best = None
        for h, disc, e, c, _en, block in play_options(hand, discard, enemy, combo, ENERGY, 0):
            out = [0.0] * (MAX_TURN + 1)
            if e <= 0:
                out[turn - 1] = 1.0
                cand = tuple(out)
                ...
                if hp2 <= 0:
                    ...
                    for nh, ndraw, ndiscard, p in next_draws(draw, discard2, hand_size):
                        fut = solve(turn + 1, nh, ndraw, ndiscard, hp2, e, c)
...
def classify(deck: tuple[str, ...]) -> str:
    c = Counter(deck)
    if c["Finisher"] and c["Twin Cut"] and c["Quick Stab"] >= 1:
        return "连击终结线"
    if c["Finisher"] and c["Setup"]:
        return "架势蓄力线"
    if c["Heavy Slash"] and c["Finisher"]:
        return "重斩混合线"
    if c["Guard"] >= 2 and c["Finisher"]:
        return "护身慢线"
    if c["Heavy Slash"]:
        return "重斩诱导线"
    return "基础混合线"
...
def main():
    ...
    parser.add_argument("--enemy-hp", type=int, default=70)
    parser.add_argument("--player-hp", type=int, default=13)
    parser.add_argument("--incoming", default="6,11,15,20")
    parser.add_argument("--output", default="set2_d1_combo_audit.json")
