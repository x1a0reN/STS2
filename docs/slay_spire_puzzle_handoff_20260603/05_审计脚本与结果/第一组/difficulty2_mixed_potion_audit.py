...
def unique_decks(pool: tuple[str, ...], min_size: int, max_size: int) -> list[tuple[str, ...]]:
    ...
    for size in range(min_size, max_size + 1):
        for pick in combinations(range(len(pool)), size):
            decks.add(deck_signature(tuple(pool[i] for i in pick)))
    return sorted(decks)


def remove_one(cards: tuple[str, ...], target: str) -> tuple[str, ...]:
    out = list(cards)
    out.remove(target)
...
def next_turn_draws(
    ...
    if len(draw_pile) >= hand_size:
        return tuple(
            (hand, next_draw, discard_pile, prob)
            for hand, next_draw, prob in draw_outcomes(draw_pile, hand_size)
        )
    drawn_fixed = draw_pile
    need = hand_size - len(draw_pile)
    if need <= 0:
        return ((tuple(sorted(drawn_fixed)), (), discard_pile, 1.0),)
    if not discard_pile:
        return ((tuple(sorted(drawn_fixed)), (), (), 1.0),)
    out = []
    for redraw, next_draw, prob in draw_outcomes(discard_pile, need):
        out.append((tuple(sorted(drawn_fixed + redraw)), next_draw, (), prob))
    return tuple(out)
...
def exact_result(deck: tuple[str, ...], variant: Variant) -> tuple[float, ...]:
    ...
    def end_turn(
        ...
        end_enemy_hp = enemy_hp - 3 * lightning
        if end_enemy_hp <= 0:
            vec[turn - 1] = 1.0
            return tuple(vec)
        if poison > 0:
    ...
    def in_turn_best(
        ...
        for idx, card in enumerate(hand):
            ...
            if cost > energy:
                continue
            if card == "Dualcast" and lightning <= 0:
                continue
    ...
    for hand, draw_pile, prob in draw_outcomes(deck, hand_size):
        result = value(1, hand, draw_pile, (), variant.player_hp, variant.enemy_hp, 0, 0, 0, 0, False)
...
def main() -> None:
    ...
    variant = Variant(
        name="difficulty2_mixed_potion",
        card_pool=pool,
        enemy_hp=72,
        player_hp=18,
        enemy_damage_by_turn=(0, 10, 14, 18),
        min_deck_size=8,
        max_deck_size=8,
    )
    rows = []
    for deck in unique_decks(pool, variant.min_deck_size, variant.max_deck_size):
    ...
    with open("difficulty2_mixed_potion_audit.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print("legal_deck_count", len(rows))
    ...
    print("best_by_turn")
    for turn in sorted(by_turn, key=lambda x: 99 if x == "None" else int(x)):
        best = by_turn[turn][0]
        print(
            turn,
            f"{best['success'] * 100:.4f}%",
            [round(p * 100, 4) for p in best["result"]],
            best["deck_display"],
        )
